...
function ExposureBadgeImpl({ container, t, compact }: Props) {
  const scope: ExposureScope = container.exposureScope ?? containerExposureScope(container)
  const routes =
    container.externalExposure?.length
      ? container.externalExposure
      : routesForContainer(container)
  ...
  return (
    <span
      ...
      title={routes.map((r) => `${r.hostIP}:${r.hostPort} → ${r.containerPort}`).join('\n')}
    >
...
export const ExposureBadge = memo(ExposureBadgeImpl)
